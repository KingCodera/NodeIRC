...
function moduleClient(name, code, options) {
    ...
    this.readMessage = function(data) {
        ...
        if (message.IRCType === 'raw') {
            return this.emitter.emit(['raw', message.command.toUpperCase()].join(':'), message);
        }
    ...
    this.awaitRaw = function(rawMessage, cb) {
        this.emitter.once(['raw', 'RPL_' + rawMessage.split(' ')[0]].join(':'), cb.bind(this));
        return this.send(
            lodash.defaults({
                IRCType: 'awaitraw',
                message: rawMessage
