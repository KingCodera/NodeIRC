...
controller.prototype.handler = function(client) {
    ...
    client.on('end', this.clientEnd.bind(this));
    client.on('error', this.clientEnd.bind(this));
...
controller.prototype.clientEnd = function(c) {
    if (c.module) {
        try {
            c.module.disconnect();
        }
        catch (error) { }
        _.remove(this.modules, c.module);
        this.logger.info('Module closed: ' + c.module.options.moduleCode);
        c.module = null;
    }
...
controller.prototype.clientData = function(c, data) {
    ...
    if (message.type === 'module') {
        ...
        newModule.client.write(JSON.stringify({IRCType: 'success'}));
