...
controller.prototype.disconnect = function() {    
    for (var i in this.clients) {
        ...
        client.end(JSON.stringify({type: 'error', message: 'Server shut down'}));
    }
    this.listener.close();
}
...
controller.prototype.clientData = function(c, data) {
    ...
    if (message.IRCType === 'awaitraw') {
        message.IRCType = 'raw';
        this.agent.awaiting.push('RPL_' + message.message.split(' ')[0].toUpperCase());
    }

    //Make sure the message is valid and has everything we need.
    if (message.type !== 'message' || (!message.to || !message.text) && message.IRCType !== 'raw') {
    ...
    if (message.IRCType === 'await') {
        message.IRCType = 'say';
        this.agent.awaiting.push(message.to);
    }

    //Send the message!
    //this.logger.info('Target: ' + message.to);
    switch(message.IRCType) {
        case 'raw': this.agent.client.send.apply(this.agent.client, message.message.split(' ')); break;
